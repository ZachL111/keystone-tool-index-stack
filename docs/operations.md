...
`keystone-tool-index-stack` is centered on cli tools work in Python. The technical target is: Package a Python local lab for index analysis with safe and unsafe fixtures, remediation hints, and documented operating limits.
...
The score is deterministic. The current threshold is 166. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
